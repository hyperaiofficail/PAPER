...
## 2026-02-24 - DoS via Unhandled None in UploadFile.filename
**Vulnerability:** FastAPI's `UploadFile.filename` can be `None` if the client does not provide a filename. Directly operating on it (e.g., using `.replace()`) without checking can lead to an unhandled `AttributeError`, causing a Denial of Service.
**Learning:** Always coalesce potentially absent inputs (like `file.filename or ""`) before performing string operations on them to prevent crash-based DoS.
**Prevention:** Use a safe fallback or explicit check when handling file upload attributes that aren't guaranteed to exist.
