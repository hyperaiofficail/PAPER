...
## 2026-02-24 - Overly Permissive CORS Configuration
**Vulnerability:** The application used wildcards (`*`) for `allow_methods` and `allow_headers` in its CORS configuration, which violates the principle of least privilege.
**Learning:** Using overly permissive CORS configurations can allow attackers to abuse the API using unexpected HTTP methods or custom headers, bypassing security controls.
**Prevention:** Always explicitly define the allowed HTTP methods (e.g., GET, POST) and headers in the CORS configuration to restrict access appropriately.
