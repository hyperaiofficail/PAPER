...
## 2026-02-23 - DoS via Unhandled NoneType in File Uploads
**Vulnerability:** FastAPI's `UploadFile.filename` can be `None` if the client does not provide a filename. Calling string methods like `.replace()` directly on it leads to an unhandled `AttributeError`, causing a Denial of Service (DoS) crash for the endpoint.
**Learning:** Never assume input properties from web frameworks are guaranteed to be non-null, especially for optional or client-provided metadata like filenames.
**Prevention:** Always coalesce potentially null properties (e.g., `file.filename or ""`) before performing string operations, and provide a safe fallback for missing data.
