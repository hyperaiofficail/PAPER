...
## 2026-02-23 - DoS bypass via unhandled null upload filename
**Vulnerability:** Fast API `UploadFile` handles empty upload names incorrectly, resolving as None type for property `filename` which then leads to an unhandled exception DoS risk if directly passed to `.replace()`.
**Learning:** Never assume string functions can directly be called on user input variables if they can be of a None type as this introduces severe unhandled exceptions.
**Prevention:** Explicitly check for `None` by doing `file.filename or ""` before calling string functions like `.replace()`.
