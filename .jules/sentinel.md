...
**Vulnerability:** The backend's CORSMiddleware was configured with `allow_methods=["*"]` and `allow_headers=["*"]`, allowing any HTTP method or header from allowed origins.
**Learning:** Using wildcards for methods and headers violates the principle of least privilege and increases the attack surface, potentially allowing attackers to exploit vulnerabilities or extract sensitive information.
**Prevention:** Always explicitly define the allowed HTTP methods and headers based on what the application actually needs. Avoid using `*` unless absolutely necessary.
