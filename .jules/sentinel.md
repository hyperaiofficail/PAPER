...
**Vulnerability:** The application was configured with `allow_methods=["*"]` and `allow_headers=["*"]` in the FastAPI `CORSMiddleware`. This overly permissive configuration could allow an attacker to send arbitrary methods and headers to the API.
**Learning:** Using wildcards in CORS configuration, especially for methods and headers, violates the principle of least privilege.
**Prevention:** Always explicitly define the allowed HTTP methods and headers based on what the application actually needs. Avoid using wildcards (`*`) unless absolutely necessary and secure.
