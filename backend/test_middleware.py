...
class TestContentLengthLimitMiddleware(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_post_request_chunked_capitalized(self):
        request = MagicMock()
        request.method = "POST"
        request.headers = {"Transfer-Encoding": "Chunked"}
        call_next = AsyncMock()
    ...
    async def test_post_request_chunked_comma_separated(self):
        request = MagicMock()
        request.method = "POST"
        request.headers = {"Transfer-Encoding": "chunked, gzip"}
        call_next = AsyncMock()
