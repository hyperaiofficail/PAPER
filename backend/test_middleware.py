...
class TestContentLengthLimitMiddleware(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_post_request_chunked_case_insensitive_and_mixed(self):
        for te_value in ["Chunked", "gzip, chunked", "CHUNKED", "compress, Chunked"]:
            request = MagicMock()
            request.method = "POST"
            request.headers = {"Transfer-Encoding": te_value}
            call_next = AsyncMock()

            response = await content_length_limit_middleware(request, call_next)
            self.assertIsInstance(response, MockJSONResponse)
            self.assertEqual(response.status_code, 411, f"Failed for {te_value}")
            self.assertIn(
                "Chunked transfer encoding is not allowed", response.body.decode()
            )
            call_next.assert_not_called()
