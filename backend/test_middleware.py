...
class TestContentLengthLimitMiddleware(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_post_request_chunked_bypasses(self):
        # Test case insensitivity and comma separated values
        bypasses = ["Chunked", "chunked, gzip", " chunked "]
        for bypass in bypasses:
            with self.subTest(bypass=bypass):
                request = MagicMock()
                request.method = "POST"
                request.headers = {"Transfer-Encoding": bypass}
                call_next = AsyncMock()
