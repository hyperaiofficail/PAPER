...
async def process_tool(
    ...
    if file:
        ...
        # Sanitize filename to prevent path traversal and unhandled AttributeError DoS
        # Handle case where file.filename is None
        raw_filename = file.filename or ""

        # We handle both / and \ as separators
        filename = os.path.basename(raw_filename.replace("\\", "/")).strip()

        # Prevent completely empty or basic traversal names after sanitization
