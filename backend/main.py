...
async def process_tool(
    ...
    if file:
        ...
        # Sanitize filename to prevent path traversal and unhandled None type DoS
        # FastAPI UploadFile.filename can be None if not provided by client
        raw_filename = file.filename or ""
        # We handle both / and \ as separators
        filename = os.path.basename(raw_filename.replace("\\", "/")).strip()

        # Fallback for empty or dangerous edge-cases
