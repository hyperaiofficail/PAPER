...
async def process_tool(
    ...
    if file:
        ...
        # Sanitize filename to prevent path traversal and DoS via None filename
        # file.filename can be None, causing AttributeError on replace
        raw_filename = file.filename or ""
        # We handle both / and \ as separators
        filename = os.path.basename(raw_filename.replace("\\", "/")).strip()

        # Fallback to safe default if filename is empty or dangerous
